...
import {
  ...
  fetchProject,
  pollProjectStatus,
  cancelStoryboardProject,
  retryStoryboardProject,
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1">
              ...
              <span className="text-xs" style={{ color: "var(--text-dim)" }}>
                {proj.total_scenes} scenes · {proj.total_characters} characters
              </span>
            </div>
          </div>
          {isProcessing && (
            <Button variant="danger" size="sm" onClick={() => cancelStoryboardProject(projectId).then(() => fetchProject(projectId))}>
              Cancel
            </Button>
          )}
          {isError && (
            <Button size="sm" onClick={() => retryStoryboardProject(projectId).then(() => fetchProject(projectId))}>
              Retry
            </Button>
          )}
        </div>
        ...
        {isProcessing && progress && (
          ...
          >
            ...
            <div className="flex items-center gap-4 mt-3">
              {progress.scenes_total > 0 && (
                <span className="text-xs" style={{ color: "var(--text-muted)" }}>
                  Scenes: {progress.scenes_processed}/{progress.scenes_total}
                </span>
              )}
              {progress.cost_usd > 0 && (
                <span className="text-xs" style={{ color: "var(--text-dim)" }}>
        ...
        {proj.status === "ready" && (
          <>
            <div
              ...
              style={{ padding: 4, borderRadius: 10, backgroundColor: "var(--surface-0)", border: "2px solid var(--border)" }}
            >
              {TABS.map((t) => (
                <button
                  key={t}
                  ...
                  style={{
                    backgroundColor: tab === t ? "var(--surface-2)" : "transparent",
                    color: tab === t ? "var(--text)" : "var(--text-muted)",
                    border: tab === t ? "2px solid var(--accent)" : "2px solid transparent",
                  }}
                >
                  {t}
                </button>
            ...
            {showWalkthrough && tab === "overview" && (
              <StoryboardWalkthrough
                ...
                onDismiss={() => {
                  ...
                  localStorage.setItem("g5-walkthrough-dismissed", "1");
                }}
              />
            )}
    ...
    <div className="space-y-2">
      {(project.scenes || []).map((scene) => (
        ...
        >
          ...
          >
            <span
              className="text-xs font-mono px-2 py-0.5 rounded min-w-10 text-center"
              style={{ color: "var(--text-dim)", backgroundColor: "var(--surface-0)", border: "2px solid var(--surface-3)" }}
            >
              {scene.scene_number}
            </span>
            ...
            <svg
              className={cn("w-4 h-4 transition-transform", expanded === scene.scene_number && "rotate-180")}
          ...
          {expanded === scene.scene_number && <SceneProduction scene={scene} project={project} projectId={projectId} />}
        </div>
      ))}
    </div>
  );
    ...
    <div style={{ padding: "16px 16px 20px", borderTop: "2px solid var(--border)" }}>
      ...
      {scene.dialogue?.length > 0 && (
        <div className="mb-4 p-3 rounded-lg" style={{ backgroundColor: "var(--surface-0)", border: "2px solid var(--surface-3)" }}>
          <span className="text-[10px] uppercase font-medium" style={{ color: "var(--text-dim)" }}>
            Dialogue
          </span>
          <div className="mt-2 space-y-1">
            {scene.dialogue.slice(0, 5).map((d, i) => (
              <p key={i} className="text-xs">
                <span style={{ color: "var(--accent-tertiary)", fontWeight: 600 }}>{d.character_name}: </span>
                <span style={{ color: "var(--text)" }}>{d.text}</span>
              </p>
            ))}
            {scene.dialogue.length > 5 && (
              <p className="text-[10px]" style={{ color: "var(--text-dim)" }}>
                +{scene.dialogue.length - 5} more lines
              </p>
            )}
          </div>
        </div>
        ...
        {[
          ...
          <div
            ...
            style={{
              ...
              padding: "8px 4px",
              borderRadius: 8,
              border: `2px solid ${s.done ? "var(--accent)" : "var(--surface-3)"}`,
              backgroundColor: s.done ? "rgba(var(--accent-rgb, 0,0,0), 0.05)" : "var(--surface-0)",
            }}
          >
            <div style={{ fontSize: 14, marginBottom: 2 }}>{s.done ? "✓" : "○"}</div>
            <div className="text-[9px] font-medium" style={{ color: s.done ? "var(--accent)" : "var(--text-dim)" }}>
              {s.label}
            </div>
            {s.text && (
      ...
      {(hasAssembled || hasSceneVideo) && (
        <div style={{ marginBottom: 16, borderRadius: 10, overflow: "hidden", border: "3px solid var(--border)" }}>
          <video
            src={scene.assembled_url || scene.scene_video_url}
            controls
            style={{ width: "100%", maxHeight: 350, backgroundColor: "#000", display: "block" }}
          />
          <div style={{ padding: 8, backgroundColor: "var(--surface-1)", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span className="text-[10px]" style={{ color: "var(--text-muted)" }}>
              {hasAssembled ? "Final scene (video + audio)" : "Scene video clip"}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div style={sectionStyle}>
          ...
          {hasVoiceover && !voiceoverValid && (
            <p className="text-[10px]" style={{ color: "var(--status-error, #ef4444)" }}>
              Audio URL expired. Click Regenerate.
            </p>
          )}
        </div>
      ...
      {/* Error */}
      {genError && (
        <p className="text-xs mt-3" style={{ color: "var(--status-error, #ef4444)" }}>
          {genError}
        </p>
      )}
      ...
      {scene.shots?.length > 0 && <ShotBreakdown shots={scene.shots} />}
    </div>
  );
}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 10, marginTop: 10 }}>
          {shots.map((shot) => (
            <div key={shot.shot_number} style={{ border: "2px solid var(--surface-3)", borderRadius: 8, padding: "10px 12px" }}>
              ...
              {shot.rationale && (
                <p className="text-[10px] italic mt-1" style={{ color: "var(--text-dim)" }}>
                  {shot.rationale}
                </p>
              )}
